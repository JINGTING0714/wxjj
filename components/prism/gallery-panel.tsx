...
export function GalleryPanel({ onOpenCollage }: { onOpenCollage: () => void }) {
  ...
  const [collections, setCollections] =
    useState<CollectionRecord[]>(coreCollections);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([storedCollections, metadata]) => {
        const merged = [
          ...coreCollections,
          ...storedCollections.filter(
            (collection) =>
              !coreCollections.some((core) => core.id === collection.id),
          ),
        ];
        const metaById = new Map(metadata.map((item) => [item.id, item]));
        const loaded = (
          await Promise.all(
            merged.map(async (collection) => {
        ...
        if (!merged.some((collection) => collection.id === active))
          setActive('daily');
      })
      .catch((reason) =>
        setError(reason instanceof Error ? reason.message : '图库读取失败'),
      );
  ...
  const removeCollection = async (id: string) => {
    ...
    if (
      !window.confirm(
        `删除图库“${collection?.name || ''}”？其中图片会移入“每日刷图”，不会被删除。`,
      )
    )
      return;
    ...
    for (const image of moving) {
      ...
      await vault.saveRecord('gallery-image-meta', {
        ...image,
        url: undefined,
        blob: undefined,
        collection: 'daily',
        updatedAt: new Date().toISOString(),
      } as unknown as GalleryImageMeta);
    }
    await vault.deleteRecord(id);
    setImages((current) =>
      current.map((image) =>
        image.collection === id ? { ...image, collection: 'daily' } : image,
      ),
    );
  ...
  const deleteImages = async (targets: GalleryImage[]) => {
    if (
      !targets.length ||
      !window.confirm(
        `确定删除选中的 ${targets.length} 张图片吗？此操作会删除本机加密副本。`,
      )
    )
      return;
    try {
      ...
      setError(e instanceof Error ? e.message : '删除失败，图片已保留。');
    }
  };
  const removeImages = async (targets: GalleryImage[]) => {
    await vault.writeBatch({
      deleteRecords: targets.map((i) => i.id),
      deleteBlobs: targets.map((i) => i.id),
    });
    for (const image of targets) URL.revokeObjectURL(image.url);
  ...
  const saveImage = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const collection = String(
      data.get('collection') || editingImage.collection,
    );
    const next: GalleryImage = {
      ...editingImage,
      name,
  ...
  return (
    <div className="studio-page gallery-page">
      <SectionHead
        ...
        number="06"
        title="图片收纳"
        description="图库和图片都能自由创建、重命名、移动、补充信息与删除。"
        actions={
          <>
            <Button
            ...
            <label className={`button-file ${busy ? 'is-disabled' : ''}`}>
              <Upload /> {busy ? '正在加密…' : '上传图片'}
              <input
                accept="image/*"
                disabled={busy}
                multiple
      ...
      {vault.status !== 'unlocked' && (
        <div className="vault-gate">
          <ShieldCheck />
          <p>
            <strong>图库正在安全锁定</strong>
            <span>先创建或解锁本机保险库，才能把图片加密写入此设备。</span>
          </p>
        </div>
      )}
      {error && (
      ...
      <div className="gallery-layout">
        <aside className="gallery-libraries">
          ...
          {collections.map((collection, index) => (
            ...
            >
              ...
              <small>
                {
                  images.filter((image) => image.collection === collection.id)
                    .length
                }
              </small>
              {collection.id !== 'daily' && (
                <span className="gallery-collection-actions">
                  <Pencil
        ...
        <section className="gallery-surface">
          <BulkActions
            ...
            disabled={busy}
            onDelete={(ids) =>
              removeImages(images.filter((i) => ids.includes(i.id)))
            }
          />
          <div className="gallery-surface-head">
            <div>
              <p className="eyebrow">CURRENT LIBRARY</p>
              <h2>{currentCollection?.name}</h2>
            </div>
            <div className="gallery-head-actions">
              <Badge variant="outline">{visible.length} IMAGES</Badge>
              {visible.length > 0 && (
                <Button
                  onClick={() =>
                    downloadZip(
                      ...
                      `${currentCollection?.name || 'PRISM-图库'}.zip`,
                    )
                  }
                  size="sm"
                  variant="outline"
          ...
          {visible.length ? (
              ...
              {visible.map((image, index) => (
                ...
                >
                  ...
                  <div className="gallery-card-actions">
                    ...
                    <button onClick={() => deleteImages([image])} type="button">
                      <Trash2 /> 删除
                    </button>
                    <button
            ...
            <label className="gallery-drop gallery-drop-empty">
              <Upload />
              <strong>把今天的图放进来</strong>
              <span>支持批量选择；图片只保存在本机加密保险库</span>
              <input
                accept="image/*"
                multiple
                onChange={(event) => upload(event.target.files)}
          ...
          <div className="gallery-footer">
            <p>
              <ShieldCheck /> 已选择 {selected.size} 张；所有处理都在本机完成。
            </p>
            <div>
              {selected.size > 0 && (
                <Button
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {collectionDialog?.id ? '重命名图库' : '新建图片库'}
            </DialogTitle>
            <DialogDescription>
              图库只是分类；图库和图片都可以以后继续修改。
            </DialogDescription>
          </DialogHeader>
          <form
          ...
          <DialogFooter>
            ...
            <Button form="gallery-form" type="submit">
              保存图库
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="image-edit-dialog">
          {editingImage && (
            <>
              ...
              <img alt={editingImage.name} src={editingImage.url} />
              <form
              ...
              >
                ...
                <label>
                  <span>所属图库</span>
                  <select
                    defaultValue={editingImage.collection}
                    name="collection"
                  >
                ...
                <label className="wide-field">
                  <span>备注</span>
                  <textarea defaultValue={editingImage.note} name="note" />
                </label>
              </form>
              <DialogFooter>
                <Button
                  onClick={() => deleteImages([editingImage])}
                  variant="outline"
                >
                  <Trash2 /> 删除图片
                </Button>
                <Button form="image-form" type="submit">
                  保存修改
                </Button>
              </DialogFooter>
            </>
